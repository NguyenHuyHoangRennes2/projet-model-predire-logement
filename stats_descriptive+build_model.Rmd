...
**Exclusion des annonces non classées énergétiquement** 
Les logements portant la mention *NC* (Non Classé) ont été retirés.
...
```{r, echo=FALSE}
reg_simple <- lm(Loyer_TCC ~ Surface + Charges + Pièces + label_eco + kWh + kgCO2 + Quartiers_Centre + Quartiers_Nord_Est + Quartiers_Nord_Ouest + Quartiers_Ouest , data = annonces)
summary(reg_simple)
```
...
L’observation des nuages de points permet d’étudier la nature de la relation entre le loyer toutes charges comprises (**Loyer\_TCC**) et les différentes variables explicatives disponibles dans la base de données.  
...
full.model <- lm(Loyer_TCC ~ Surface + Charges + Pièces + 
                label_eco + kWh + kgCO2 +
...
```{r, echo=FALSE}
# estimation du modèle complet 
full.model <- lm(Loyer_TCC ~  Surface +Charges + Charges**2 + Pièces + label_eco + label_eco**2+
...
modele2 <- lm(log(Loyer_TCC) ~ log(Surface) + Charges + Pièces + 
                label_eco + kWh + kgCO2 +
                Quartiers_Centre + Quartiers_Nord_Est + 
                Quartiers_Nord_Ouest , 
              data = annonces)
# Modèle 3: Avec termes quadratiques pour Charges, kWh, kgCO2
modele3 <- lm(Loyer_TCC ~ Surface + Charges + I(Charges^2) + Pièces + 
...
modele4 <- lm(Loyer_TCC ~ Pièces + Charges + 
